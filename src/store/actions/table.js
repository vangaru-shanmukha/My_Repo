...
import { USER_NAME_SESSION_ATTRIBUTE_NAME } from '../../Components/Login/AuthenticationService';
import { TOKEN } from '../../Components/Login/AuthenticationService';
    ...
    if(qty < actualQty) {
        ...
        for(let order in orders) {
            if(item === orders[order].item.name && actualQty > qty) {
                promisesArray.push(
                    /* axios.delete('http://localhost:8080/orders/' + orders[order].id) */
        ...
        for(let order in orders) {
            if(item === orders[order].item.name && actualQty < qty) {
                ...
                promisesArray.push(
                    /* axios.post('http://localhost:8080/orders/', orders[order]) */
