...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i=0;i<word.length();i++)
		{
			...
			if((i == word.length()-1) && Character.toString(c).toLowerCase().equals("e"))
			{
				if(count == 0)
					count++;
			}
			else if(isVowel(c) == true && flag == false)
			{
				count++;
				flag = true;
			}
	...
	/*
	 * This method is used to check whether the character c is a syllable or not
	 */
	private boolean isVowel(char c)
	{
		String s = c+"";
		if("aeiouy".contains(s) || "AEIOUY".contains(s))
			return true;
		return false;
	}
